...
current_context_session: ContextVar[Optional["PromptContextSession"]] = ContextVar(
    "current_context_session",
    default=None,
)
...
class PromptContextSession(AbstractContextManager):
    ...
    def __init__(
        ...
        :param memory: Optional memory backend or config (e.g., "redis" or a config
            dict).
        :param cache_ttl: Optional cache TTL for context (not implemented in this
            pure version).
    ...
    def add_memory(self, content: str, metadata: Optional[dict] = None) -> None:
        """Add a piece of contextual memory (e.g., prior conversation or fact)."""
    ...
    def add_tool(self, name: str, description: str = "") -> None:
        """Register an available tool in this context (for MCP tool usage)."""
        tool = ToolSpec(name=name, description=description)
        self.tools.append(tool)

    def build(self, prompt: str) -> str:
        """
        Build an enhanced prompt by injecting collected context (memory, negative
        examples, etc.) into the original prompt. Returns the combined prompt string.
        """
        # If RAG is enabled, trigger hook before performing retrieval
        # (e.g., to fetch external context).
        if self.rag_enabled:
            hook_registry.notify("before_rag_query", query=prompt, context=self)
            # (A hook could populate self.rag_results or self.memory_items with
            # retrieved data.)
        ...
        if len(self.memory_items) > 50:
            ...
            # Simple compression: keep only the last 50 memory items (placeholder
